...
const 
  ...
  IS_SEND_FILES_IN_BODY = false, // добавить ссылки на файлы в тело письма
  ...
  MAIL_ADDRESS = 'ilyer84@ya.ru', // кому необходимо отправить письмо
...
$data['result'] = 'success';
echo '<br>$data[result]= '.$data['result'].'<br>';
/* 4 ЭТАП - ВАЛИДАЦИЯ ДАННЫХ (ЗНАЧЕНИЙ ПОЛЕЙ ФОРМЫ) */
// проверка поля name
if (isset($_POST['button1'])) {
    echo 'button1= '.$data['result'].'<br>';
}
if (isset($_POST['name'])) {
...
if ($data['result'] == 'success' ) { //if ($data['result'] == 'success' && IS_SEND_MAIL == true) { 
  // получаем содержимое email шаблона
  $bodyMail = file_get_contents(__DIR__ .'/email.tpl');
