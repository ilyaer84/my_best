...
var ProcessForm = function(config) {
    ...
    for (var prop in config) { // Перебрать свойства объекта, для каждого свойства выполнить заданный код
        // prop  Переменная, которой последовательно присваиваются названия свойств объекта.
...
ProcessForm.prototype = function() {
    ...
    //Метод .prop() получает значение свойства первого найденного элемента
    ...
    var _validateForm = function(form) {
        ...
        $(form).find('input, textarea').not('[type="file"], [name="agree"]').each(function() {
            ...
            if (this.checkValidity()) {
                //  Метод .checkValidity(). В случае, когда встроенная валидация возможна, для проверки полей может быть использован метод
    ...
    var _showForm = function(_this) {
        var
            form = _this.getForm(),
            $form = $(form);
        if (!$form.find('.form-error').hasClass('d-none')) { // Метод .hasClass() возвращает true если у элемента есть искомый класс и false в обратном случае.
            // find возвращает значение первого элемента в массиве, который соответствует условию в переданной функции, или undefined, если 
        ...
        $form.find('input, textarea').each(function() {
            // each перебираются все DOM-элементы включенные в выборку объекта
    ...
    var _changeInputFile = function(e, _this) {
        $(e.currentTarget)
            // Свойство currentTarget всегда ссылается на элемент, к которому был присоединен обработчик события, а не на элемент, в котором произошло событие.
            .removeClass('is-invalid is-valid')
            .parent()
            // метод .parent() возвращает родительский элемент каждого элемента в наборе совпавших элементов, дополнительно может фильтроваться с помощью заданного селектора.перемещается только на один уровень вверх по дереву DOM
            ...
            .text('');
        // text Возвращает или изменяет текстовое содержимое выбранных элементов
    ...
    var _sendForm = function(_this, config) {
        ...
        $.ajax({
            context: _this,
            type: "POST", //$(_this).attr('method'), //
            //       url: window.wp_data.ajax_url,
            url: base_url + '/wp-content/themes/test/assets/modul/process.php',
        ...
        .done(_success)
            // done добавляем обработчик при успешном выполнении запроса
            .fail(_error)
            // добавляем обработчик при неудачном выполнении запроса
    };
    ...
    var _init = function() {
        ...
        this.setIsAgreement($(this.getForm()).find('.agreement').length > 0); // имеется ли у формы секция agreement Пользовательское солашение
        this.setIsAttachments($(this.getForm()).find('.attachments').length > 0); // имеется ли у формы секция attachments Файлы
        _setupListener(this);
    }
    ...
    var _setupListener = function(_this) {
        ...
        $(document).on('submit', _this.getConfig().selector, function(e) {
            e.preventDefault();
            //e.preventDefault(); убиваем отправку из браузера
